...
from writhe_tools.writhe_nn import AddWritheEdges
...
class PaiNNBase(torch.nn.Module):
    ...
    def __init__(
            ...
            writhe_layer: bool = False,
            cross_product: bool = False,
            n_atoms: int = 20
    ):

        super().__init__()
        # this let's us decide between WrithePaiNN chiro-PaiNN and PaiNN
        message = SE3WritheMessage if writhe_layer else SE3Message if cross_product else Message
        layers = []
...
class SE3WritheMessage(nn.Module):
    def __init__(
        ...
        self.positional_encoder = PositionalEncoder(int(self.n_features // 2), length=length_scale)
    ...
    def forward(self, batch):
        ...
        writhe = batch.writhe.flatten()
        writhe = torch.stack([torch.sin(writhe / 10 * i * torch.pi)
                              for i in torch.arange(int(self.n_features // 2))],
                              dim=-1)

        # noinspection PyCallingNonCallable
        gates, scale_edge_dir, ds, de, cross_product_gates = torch.split(
            self.phi(in_features) * self.w(torch.cat([positional_encoding, writhe], dim=-1)),
            self.n_features,
            dim=-1,
        )
        ...
        gated_cross_products = multiply_first_dim(cross_product_gates,
                                                  batch.vector.reshape(-1, 1, 3).repeat(
                                                      1, self.n_features, 1)
                                                 )
...
class Update(torch.nn.Module):
    def __init__(self, n_features=128):
        ...
        self.n_features = n_features
        self.mlp = MLP(2 * n_features, n_features, 3 * n_features) # in features should be 3 if writhe

    def get_writhe_features(self, batch):
        """
        To obtain a set of writhe features for each node, we do weighted message passing
        using attention weights = 1 / distance^2 to match the decay of the writhe.
        The writhe features are embedding using sines as done in the message block.
        """
        dst_node = batch.edge_index[1]
        weights = 1 / batch.edge_dist**2 #overwrite this with the actual weights for mem conservation
        weights = (weights / scatter(weights, dst_node)[dst_node]).unsqueeze(-1) # normalize weights
        return scatter(torch.stack([torch.sin(batch.writhe / 10 * i * torch.pi)
                                    for i in torch.arange(self.n_features)], dim=-1)
                       * weights, dst_node, dim=0)
    ...
    def forward(self, batch):
        ...
        mlp_in = torch.cat([vv_norm, s], dim=-1) # , self.get_writhe_features(batch)
...
class PaiNNScore(torch.nn.Module):
    ...
    def __init__(
        ...
        layers = [
            ...
            AddWritheEdges(add_vector=True) if writhe_layer else nn.Identity(),
